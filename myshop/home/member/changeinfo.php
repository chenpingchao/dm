...
//查询该用户的信息
$member = oneSelect("select mobile,email,avatar from member where id=".$_SESSION['mid']." limit 1");
...
<body>
<div class="main">
	<p class="title"><span>更改个人信息</span></p>
    <form action="memberAction.php?act=upinfo" method="post" enctype="multipart/form-data" id="form1">
	<table cellpadding="0" cellspacing="0" width="100%">
    	<tr>
    		<td class="textright">用户名：</td>
    		<td><input type="text" disabled value="<?php echo $_SESSION['mname']?>" /></td>
    	</tr>
    	<tr>
    		<td class="textright">手机号：</td>
    		<td><input type="text" value="<?php echo $member['mobile']?>" name="mobile" /></td>
    	</tr>
    	<tr>
    		<td class="textright">邮箱：</td>
    		<td class="pwd"><input type="text" value="<?php echo $member['email']?>" name="email"/></td>
    	</tr>
    	<tr>
    		<td class="textright">头像：</td>
    		<td><img src="/upload/<?php echo $member['avatar'] ? $member['avatar'] : ''?>" alt="无图片" style="width:70px;border-radius:50px;"><input type="file" name="avatar" /></td>
    	</tr>
    	<tr>
    		<td colspan="2">
    			<a href="#" class="btns" onclick="document.getElementById('form1').submit()">保存</a>
    		</td>
    	</tr>
    </table>
    </form>
