...
//$type代表返回的字符串类型 1表示纯数字  2表示小写字母  3表示大写字母 4表示大小写混合 5表示字母和数字混合 6表示汉字
 public function get_rand_str($type=1,$length=4,$str="我是这一个中国人是世界上最好的语言一二三四五六七"){
    switch($type){
        case 1:
            $str = join('',range(0,9));
            break;
...
*/
public function captcha($w,$h,$length=4,$type=1,$str='',$size=14,$fonts=['fonts/msyh.ttf','fonts/msyhbd.ttf','fonts/simkai.ttf']){
    //第一步：创建画布，获取图像资源
    ...
    //第二步：操作图像资源
     ...
     for($c=0;$c<$length;$c++){
         ...
         //文字
         $text = $this->get_rand_str($type,1,$str);
         $captcha .=  $text;
         //绘制文字
...
//文字水印
public function textWater($des_img,$text,$size=28,$angle=0,$x=0,$y=0,$color=[255,0,0],$font="fonts/msyhbd.ttf",$dir='upload'){
   //第一步：将图片转换为图像资源
    //获取图片的信息
...
//图片水印
public function imageWater($des_img,$water_img,$x=0,$y=0,$alpha=30,$dir='upload'){
    //由目标图像产生目标图像资源
...
//等比缩放
public function thumb($src_img,$w,$dir='upload'){
...
//剪裁
public function cut($src_img,$x,$y,$w,$h,$dir='upload'){
